...
//  @OnlyIn(Dist.CLIENT)
//  override def getArmorModel(entityLiving: LivingEntity, itemStack: ItemStack, armorSlot: EquipmentSlot, _default: HumanoidModel[_]): HumanoidModel[_] = {
//    if (armorSlot == slot) {
//      HoverBootRenderer.lightColor = if (ItemColorizer.hasColor(itemStack)) ItemColorizer.getColor(itemStack) else 0x66DD55
//      HoverBootRenderer.asInstanceOf[HumanoidModel[_]]
//    }
//    else
...
//  }
  ...
  override def getDamage(stack: ItemStack): Int = {
    val data = new HoverBootsData(stack)
    this.getMaxDamage(stack: ItemStack) - data.charge.toInt
  }
