        ...
        case Some(header: Array[Byte]) if new String(header, Charsets.UTF_8) == "nanomachines" =>
          ...
          command match {
            ...
            case Array("saveConfiguration") =>
              ...
              try {
                val index = player.getInventory.items.indexWhere(stack => api.Items.get(stack) == nanomachines && new NanomachineData(stack).configuration.isEmpty)
                if (index >= 0) {
                  val stack = player.getInventory.removeItem(index, 1)
