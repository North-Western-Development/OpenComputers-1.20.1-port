...
class Adapter(props: Properties) extends SimpleBlock(props) with traits.GUI {
  ...
  override def newBlockEntity(pos: BlockPos, state: BlockState) = new tileentity.Adapter(tileentity.TileEntityTypes.ADAPTER.get(), pos, state)
  ...
  override def onNeighborChange(state: BlockState, world: LevelReader, pos: BlockPos, neighbor: BlockPos): Unit =
    world.getBlockEntity(pos) match {
