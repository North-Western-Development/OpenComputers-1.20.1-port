...
class PowerDistributor(props: Properties) extends SimpleBlock(props) {
  override def newBlockEntity(pos:BlockPos, state: BlockState) = new tileentity.PowerDistributor(tileentity.BlockEntityTypes.POWER_DISTRIBUTOR.get(), pos, state)
  ...
  override def getTicker[T <: BlockEntity](level: Level, blockState: BlockState, blockEntityType: BlockEntityType[T]): BlockEntityTicker[T] = {
    (_: Level, pos: BlockPos, state: BlockState, entity: T) =>
      entity match {
        case tileEntity: tileentity.PowerDistributor => tileEntity.updateEntity()
        case _ =>
      }
  }
}
