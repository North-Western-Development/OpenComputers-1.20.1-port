...
class DiskDrive(props: Properties) extends SimpleBlock(props) with traits.GUI {
  ...
  override def newBlockEntity(pos:BlockPos, state: BlockState) = new tileentity.DiskDrive(tileentity.BlockEntityTypes.DISK_DRIVE.get(), pos, state)
  ...
  override def getTicker[T <: BlockEntity](level: Level, blockState: BlockState, blockEntityType: BlockEntityType[T]): BlockEntityTicker[T] = {
    (_: Level, pos: BlockPos, state: BlockState, entity: T) =>
      entity match {
        case tileEntity: tileentity.DiskDrive => tileEntity.updateEntity()
        case _ =>
      }
  }
