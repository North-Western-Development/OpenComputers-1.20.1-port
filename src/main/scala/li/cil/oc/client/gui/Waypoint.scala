...
class Waypoint(val waypoint: tileentity.Waypoint) extends screens.Screen(Component.empty()) {
  val imageWidth = 176
  val imageHeight = 24
  var leftPos = 0
  var topPos = 0
  ...
  override def render(stack: GuiGraphics, mouseX: Int, mouseY: Int, dt: Float): Unit = {
    super.render(stack, mouseX, mouseY, dt)
    RenderSystem.setShaderColor(1, 1, 1, 1) // Required under Linux.
    minecraft.getTextureManager.bindForSetup(Textures.GUI.Waypoint)
