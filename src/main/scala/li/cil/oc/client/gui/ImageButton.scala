...
class ImageButton(xPos: Int, yPos: Int, w: Int, h: Int,
  ...
  override def renderWidget(guiGraphics: GuiGraphics, mouseX: Int, mouseY: Int, partialTicks: Float) {
    if (visible) {
      Textures.bind(image)
      ...
      if (getMessage != Component.empty()) {
        ...
        if (textIndent >= 0) guiGraphics.drawString(mc.font, getMessage, textIndent + x, y + (height - 8) / 2, color)
        else guiGraphics.drawCenteredString(mc.font, getMessage, x + width / 2, y + (height - 8) / 2, color)
      }
    }
  }
}
