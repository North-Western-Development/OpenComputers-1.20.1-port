...
class Drive(playerInventory:Inventory, val driveStack: () => ItemStack) extends screen.Screen(StringTextComponent.EMPTY) with traits.Window {
  ...
  override def render(stack: PoseStack, mouseX: Int, mouseY: Int, dt: Float): Unit = {
    ...
    font.drawWordWrap(Component.literal(Localization.Drive.Warning), leftPos + 11, topPos + 37, imageWidth - 20, 0x404040)
    font.drawWordWrap(Component.literal(Localization.Drive.LockWarning), leftPos + 61, topPos + windowHeight - 48, imageWidth - 68, 0x404040)
  }
}
