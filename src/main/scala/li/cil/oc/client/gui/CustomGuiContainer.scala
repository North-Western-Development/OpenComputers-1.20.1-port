...
abstract class CustomGuiContainer[C <: Container](val inventoryContainer: C, inv:Inventory, title: Component)
  ...
  override def renderWrappedToolTip(guiGraphics: GuiGraphics, text: util.List[_ <: Component], x: Int, y: Int, font: Font): Unit = {
    copiedDrawHoveringText0(guiGraphics, text, x, y, font)
  }
  ...
  protected def copiedDrawHoveringText(stack: GuiGraphics, lines: util.List[String], x: Int, y: Int, font: Font): Unit = {
  ...
  protected def copiedDrawHoveringText0(guiGraphics: GuiGraphics, text: util.List[_ <: Component], x: Int, y: Int, font: Font): Unit = {
    if (!text.isEmpty) {
      ...
      val bg = 0xF0100010
      fillGradient(guiGraphics, posX - 3, posY - 4, posX + textWidth + 3, posY - 3, bg, bg)
      fillGradient(guiGraphics, posX - 3, posY + textHeight + 3, posX + textWidth + 3, posY + textHeight + 4, bg, bg)
      fillGradient(guiGraphics, posX - 3, posY - 3, posX + textWidth + 3, posY + textHeight + 3, bg, bg)
      fillGradient(guiGraphics, posX - 4, posY - 3, posX - 3, posY + textHeight + 3, bg, bg)
      fillGradient(guiGraphics, posX + textWidth + 3, posY - 3, posX + textWidth + 4, posY + textHeight + 3, bg, bg)
      val color1 = 0x505000FF
      val color2 = (color1 & 0x00FEFEFE) >> 1 | (color1 & 0xFF000000)
      fillGradient(guiGraphics, posX - 3, posY - 3 + 1, posX - 3 + 1, posY + textHeight + 3 - 1, color1, color2)
      fillGradient(guiGraphics, posX + textWidth + 2, posY - 3 + 1, posX + textWidth + 3, posY + textHeight + 3 - 1, color1, color2)
      fillGradient(guiGraphics, posX - 3, posY - 3, posX + textWidth + 3, posY - 3 + 1, color1, color1)
      fillGradient(guiGraphics, posX - 3, posY + textHeight + 2, posX + textWidth + 3, posY + textHeight + 3, color2, color2)
  ...
  private def fillGradient(guiGraphics: GuiGraphics, left: Int, top: Int, right: Int, bottom: Int, startColor: Int, endColor: Int): Unit = {
    guiGraphics.fillGradient(left, top, right, bottom, startColor, endColor)
    RenderState.makeItBlend()
  }

  override def render(stack: GuiGraphics, mouseX: Int, mouseY: Int, partialTicks: Float): Unit = {
